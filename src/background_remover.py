...
try:
    ...
    messagebox.showerror("Startup Error", "Please launch the app with start.sh or: python src/main.py")
    sys.exit(1)
...
I18N = {
    "en": {
        ...
        "startup_error": "Startup error: please open the app using start.sh or: python src/main.py",
        ...
        "drop_not_available": "Drag and drop is unavailable. Install dependencies with start.sh or: pip install tkinterdnd2",
    },
    "pt": {
        ...
        "startup_error": "Erro de inicialização: abra o app com start.sh ou: python src/main.py",
        ...
        "drop_not_available": "Arrastar e largar indisponível. Instale as dependências com start.sh ou: pip install tkinterdnd2",
    }
}
...
def copy_image_to_clipboard(img: Image.Image):
    """Copy image to clipboard. Supports Wayland (wl-copy) and X11 (xclip/xdotool)."""
    ...
    if wayland:
        ...
        raise OSError(
            "wl-copy not found. Install it with: sudo dnf install wl-clipboard"
        )
    else:
        # X11: try xclip first, then xdotool as fallback
        ...
        if shutil.which("xdotool"):
            # xdotool doesn't support image types — write temp file and use xdg-open workaround
            raise OSError("xclip not found. Install it with: sudo dnf install xclip")
        raise OSError(
